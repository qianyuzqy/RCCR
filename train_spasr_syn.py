...
def main():
    ...
    if True:
        data_loader = get_loader('syn')
        data_path = get_data_path('syn')
        if random_crop:
    ...
    for i_iter in range(start_iteration, num_iterations):
        ...
        print('iter = {0:6d}/{1:6d}, loss_l = {2:.3f}, loss_consistency = {3:.3f}, loss_contrastive = {4:.12f}, iter time = {5:.2f} s'.format(
        ...
        if i_iter % val_per_iter == 0 and i_iter != 0:
            model.eval()
            if dataset == 'cityscapes':
                _, mIoU16, mIoU13 = evaluate(model, dataset, ignore_label=255, input_size=(512,1024), save_dir=checkpoint_dir)
                mIoU13 = mIoU13 * 100
                mIoU16 = mIoU16 * 100
                ...
                f_mIoU = open(osp.join(checkpoint_dir, 'mIoU13.txt'), 'a')
                # f_mIoU.write('{0:.4f}\n'.format(mIoU))
                f_mIoU.write('{0:d} steps: {1:.4f}\n'.format(i_iter, mIoU13))
                f_mIoU.close()            

                f_mIoU = open(osp.join(checkpoint_dir, 'mIoU16.txt'), 'a')
                # f_mIoU.write('{0:.4f}\n'.format(mIoU))
            ...
            if mIoU13 > best_mIoU16 and save_best_model:
                best_mIoU16 = mIoU16
            ...
            if config['utils']['tensorboard']:
                tensorboard_writer.add_scalar('Validation/mIoU', mIoU13, i_iter)
    ...
    model.eval()
    if dataset == 'cityscapes':
        _, _, mIoU = evaluate(model, dataset, ignore_label=255, input_size=(512,1024), save_dir=checkpoint_dir)
        mIoU = mIoU * 100
        print('Cur mIoU', mIoU)
